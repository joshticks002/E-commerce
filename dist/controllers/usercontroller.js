...
const deleteCartProduct = asyncHandler(async (req, res) => {
    ...
    if (user[0].User) {
        ...
        await Viewable.updateOne({ Uid: userId }, {
            cart: foundCart || [],
        });
        res.status(201).render("cartProduct", {
            title: "Cart",
...
const deleteAllCartProduct = asyncHandler(async (req, res) => {
    ...
    const user = await Viewable.find({ Uid: userId });
    if (user[0].User) {
        const updateUserCart = await Viewable.updateOne({ Uid: userId }, {
            cart: [],
        });
        res.status(201).render("cartProduct", {
            title: "Cart",
...
const clearCart = asyncHandler(async (req, res) => {
    ...
    if (user[0].Agent) {
        ...
        const updateUserTransaction = await Viewable.updateOne({ Uid: userId }, {
            cart: [],
        });
        res.status(201).render("cartProduct", {
            title: "Cart",
...
const buyProducts = asyncHandler(async (req, res) => {
